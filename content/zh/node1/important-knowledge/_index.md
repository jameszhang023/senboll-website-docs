...
1. 只有强制布防可忽略当前开路防区，前提是这些开路防区的强制属性（默认开启）必须是开启的。其余布防方式都必须在防区闭合状态才可以操作。
2. 强制布防的时候，要弄清楚防区开路的原因、这些开路防区不影响系统安全性的前提下才可操作。
...
枫叶主机上的继电器是可以自动通过事件组来驱动，即可编程继电器（PGM），报警系统可以在某个条件（可以通过系统编程设置触发的条件）满足的时候自动驱动继电器输出信号，用来给第三方提供信号或者控制电器。
