...
键盘是4芯总线和主机通讯，其中4根线里面含有电源和信号线，因为键盘和主机之间是串行数据通讯，通常我们建议采用标准4芯带屏蔽的线材布线。如线果没有用带屏蔽的材布线，在施工的过程中，万一和强电并行走线的时候，可能会产生干扰信号，或者附近有强大的电磁信号，都会造成通讯不畅甚至无法通讯。

## 警号布线

警号是安防系统的重要组成，对于警号的布线比较简单，采用RVV2 x 1.0+ 线材，无需带屏蔽，除去线路衰减，只要保证线路能提供足够的电源即可。
...
比如常闭（触发后输出开路信号）输出，如果忽略电阻，理论上传输距离是无限远。用4芯线连接主机和探测器即可，只要线路能满足供电要求即可，不用考虑信号的干扰问题。这种布线方式需要每一个探测器单独布线到主机或防区模块，需要的线材多，比较浪费。拓扑图如下：
![星型布线](images/star-wire.png)

### 探测器布线：总线

{{< note >}}
如果总线探测器数量超过20个，建议采用第三种方式，即混合型布线方式。
{{< /note >}}
...
如果总线模块(总线探测器也是总线模块的一种)数量比较多，通常情况如果超过20个，总线上的模块数量越多，通讯是速度会变慢，有时候甚至会无法通信。建议采用8防区模块来扩展防区，以此减少总线模块的数量，来提高可靠性和速度，这是一种最佳解决方案，如图：
...
| Keypad-K641 | 150mA |
| Keypad-K6456 | 150mA |
| Detector-NV35MX | 30mA |
| Detector-NV35M | 30mA |
| Detector-NV790 | 50mA |
| Detector-DG467 | 30mA |
| Detector-NV75M | 30mA |
| Detector-NV75MW | 30mA |
| Detector-DG85 | 50mA |
| Detector-NVX80 | 100mA |
| Detector-NV780M | 80mA |
| Detector-VE9935 | 80mA |
| Detector-VE918 | 50mA |
| Detector-VE981 | 130mA |
| Detector-VE912 | 100mA |
| Detector-960 | 30mA |
| Detector-980 | 30mA |
| Detector-VE40M | 80mA |
| Detector-VE60M | 90mA |
| Detector-VE80M | 100mA |
| Detector-VE100M | 120mA |
| Access-ACM12 | 150mA |
| Access-R915 | 150mA |
| Interface-HUB2 | 50mA |
| Interface-RTX3 | 150mA |
| Interface-VDMP3 | 150mA |
| Interface-PCS250G | 450mA |
| Interface-PCS250 | 450mA |
| Interface-IP150 | 100mA |
| Interface-PRT3 | 25mA |
| Zone-ZC1 | 30mA |
| Zone-ZX82 | 100mA |
| Relay-PGM82 | 100mA |
