...
如果含有HD77/HD78等系列视频探测器，需要填帐户名和用户密码，默认最高权限帐户名是`master`，默认用户密码是`1234`。如图：
...
步骤一：确保计算机和网络模块在同一网段下，启动枫叶搜索工具（请访问[资料服务器](http://support.senboll.com:8888)下载），查询网络模块IP地址，双击搜到的模块会弹出网页登录界面，输入用户密码将会进入网页设置界面，如图：
...
{{< note >}}
注意：如果有防区开路将无法布防，需要恢复防区状态或者旁路才能布防。
{{< /note >}}
