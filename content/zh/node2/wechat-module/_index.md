...
请提前（例如，探测器，警号，蓄电池，操作键盘）接线并对防区，分区，用户等内容进行编程设置。

#### 硬件

枫叶主机和微信模块，互联⽹（连接微信模块的路由器需要开启DHCP选项）。
...
使用微信模块包装盒里的四芯线将微信模块的Panel口和安防主机Serial口连接，保证网络畅通，插网线至微信模块Wan口，然后给整个系统上电。
...
点击{{< button >}}账户管理{{< /button >}} → {{< button >}}账号管理{{< /button >}} → {{< button >}}添加主机{{< /button >}}，扫描微信模块外壳上的二维码，并完善一些信息和选择需要的推送事件，如图：
...
回到公众号首页点击{{< button >}}账户管理{{< /button >}} → {{< button >}}充值缴费{{< /button >}}，在弹出的新页面点{{< button >}}续费{{< /button >}}。

接下来的页面中有3个选择：

- 试用一个月，点击{{< button >}}确认支付{{< /button >}}，即可开启1个月的试用期，试用期到了会停止推送，续费后将会继续提供服务。
- 推荐选择360元/三年，相当于每年120元，比较划算，充值后1个月的试用期会自动累加进去。
- 1年缴费150元，充值后1个月的试用期会自动累加进去。
...
①：资源的标签设置 - 配置分区、防区、用户、门等标签，要和主机编程的参数相匹配；  
②：成员 - 增减和设置成员权限  
③：修改 - 修改当前主机的一些配置信息，如防区和分区数量等；  
...
如果已添加了主机，需要修改当前主机的防区和分区数量等，进入修改即可，如下图：
...
请访问 → [成员管理指南](../../node2/wechat-module/#%E6%88%90%E5%91%98%E7%AE%A1%E7%90%86)
