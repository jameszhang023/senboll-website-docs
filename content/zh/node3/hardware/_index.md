...
2. 卡内的文件有问题，请访问 → [资料服务器（账户和密码咨询设备提供商）](https://support.senboll.com/)下载，根据操作键盘型号找相应的SD卡文件，**切记请先格式化内存卡**，再拷贝文件到SD卡根目录。例如：下面是TM50的文件位置，如图：
...
SD卡初始化文件请访问 → [资料服务器（账户和密码咨询设备提供商）](https://support.senboll.com/)下载，根据操作键盘型号找相应的SD卡文件，**切记请先格式化内存卡**，再拷贝文件到SD卡根目录。例如：下面是TM50的文件位置，如图：
![SD卡初始化文件](images/tm50-sd-file.png)
...
有可能是SD卡文件的问题，请下载最新的文件，SD卡初始化文件请访问 → [资料服务器（账户和密码咨询设备提供商）](https://support.senboll.com/)下载，根据操作键盘型号找相应的SD卡文件，**切记请先格式化内存卡**，再拷贝文件到SD卡根目录。
...
如果在局域网里并且知道模块的具体地址，请使用枫叶搜索工具（工具下载请访问 → [资料服务器（账户和密码咨询设备提供商）](https://support.senboll.com/)下载）搜索网络模块。
