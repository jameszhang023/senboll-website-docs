...
如果软件无法编程请使用键盘编程，具体步骤如下：

#### 通过键盘编程

步骤一 请先使用进入编程模式，具体方法如下：
...
| K656/K641+ | 长按0 → 输入安装者密码（默认是000000） |

针对SP/MG：

|键盘|步骤|
|:------|:------|
| TM70 | 菜单 → 系统设置 → 安装者密码（默认：0000） → 高级设置 → 输入0000 → 系统编程 |
| TM50 | 菜单 → 系统设置 → 安装者密码（默认：0000） → 高级设置 → 输入0000 → 系统编程 |
| K10V/K32+ | 长按0 → 输入安装者密码（默认是0000） |
...
| MG/SP | 输入687-690（第一个警号对应683，以此类推，最多支持4个） |
| MG6250 | 输入0801-0804（第一个警号对应0801，以此类推，最多支持4个） |
